...
def fallback_rag(user_id, chat_id, message, history, model_id):
    ...
    # LOG.info("="*30 + " FINAL LLM PROMPT " + "="*30)
    # ...
    # =============================================
...
def handler(event, context):
    ...
    if 'httpMethod' in event:
        api_map = {
            'POST/rag/query': lambda x: handle_rag_query(x),
            'POST/rag/delete-chat': lambda x: handle_delete_chat(x),
            'GET/rag/list-chats': lambda x: handle_list_chats(x), # Changed to GET to align with restful pattern for listing
            'GET/rag/get-chat-history': lambda x: handle_get_chat_history(x),
...
#     print("--- Starting Local Manual Test ---")
...
#         "resource": "/rag/query",
#         "body": json.dumps({
#             "user_id": "local-tester@example.com",
#             "chat_id": "manual-test-chat-01",
#             "message": "What does the document say about the exam date?", 
...
# ===== PROMPTS (merged from prompt_utils.py) =====
...
# ===== WEB SEARCH (merged from web_search.py) =====
...
def perform_web_search(query: str, num_results: int = 3) -> str:
    ...
    try:
        ...
        # Add random delay (2-5 seconds) to mimic human behavior and avoid rate limits
